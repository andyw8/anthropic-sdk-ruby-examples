...
def add_duration_to_datetime(datetime_str, duration: 0, unit: "days", input_format: "%Y-%m-%d")
  ...
  case unit
  when "seconds"
    new_date = date + Rational(duration, 86400)
  when "minutes"
    new_date = date + Rational(duration, 1440)
  when "hours"
    new_date = date + Rational(duration, 24)
  when "days"
    new_date = date + duration
  when "weeks"
    new_date = date + (duration * 7)
  when "months"
    new_date = date >> duration
  when "years"
    new_date = date >> (duration * 12)
  else
...
def set_reminder(content, timestamp)
  puts "----"
  puts "Setting the following reminder for #{timestamp}:"
  puts content
  puts "----"
  nil
end
...
ADD_DURATION_TO_DATETIME_SCHEMA = {
  ...
  "description" => "Adds a specified duration to a datetime string and returns the resulting datetime in a detailed format. This tool converts an input datetime string to a Ruby DateTime object, adds the specified duration in the requested unit, and returns a formatted string of the resulting datetime. It handles various time units including seconds, minutes, hours, days, weeks, months, and years, with special handling for month and year calculations to account for varying month lengths and leap years. The output is always returned in a detailed format that includes the day of the week, month name, day, year, and time with AM/PM indicator (e.g., 'Thursday, April 03, 2025 10:30:00 AM').",
  "input_schema" => {
    "type" => "object",
    "properties" => {
      "datetime_str" => {
  ...
  tool_requests.each do |tool_request|
    begin
      ...
      tool_result_block = {
        ...
        "content" => JSON.generate(tool_output),
        "is_error" => false
      }
    rescue => e
      tool_result_block = {
